...
export default function AuthScreen({ navigation }) {
  ...
  const handleAuth = async () => {
    try {
      ...
      // For testing on web, you can bypass Firebase auth
      if (Platform.OS === 'web' && process.env.NODE_ENV === 'development') {
        console.log('Development mode on web - bypassing auth for testing');
        // Create a test user
        const testUserId = 'test-user-' + Date.now();
        
        // For sign up mode, create a user document
        if (isSignUpMode) {
          console.log('Creating test user document');
          try {
            await setDoc(doc(firestore, 'users', testUserId), {
              email: email || 'test@example.com',
              name: childName || 'Test User',
